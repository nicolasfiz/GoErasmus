...
export const signUp = async (req, res) => {
  try {
    ...
    if (nombrePersona === '' || primerApellido === '' || nombreUsuario === '' || correo === ''
        || contrasenaSinCifrar === '' || contrasenaRepetida === '')
      return res.status(500).json({ message: "Bad Request. Please fill all fields" })
    else if (!validEmail.test(correo))
      return res.status(500).json({ message: "Invalid email" })
    else if (contrasenaSinCifrar.length < 5 || contrasenaSinCifrar.length > 20)
    return res.status(500).json({ message: "Password size must be between 5 and 20" })
    ...
    const roles = await connection.query('SELECT idRol, nombre FROM Rol WHERE nombre = "Turista"')
...
export const sendPasswordtoUserEmail = async (req, res) => {
  try {
    ...
    const emailFound = await connection.query("SELECT email FROM usuario WHERE email=?", [email])
    if (emailFound.length === 0)
        return res.status(500).json({message: "Bad request. That email isn't registered in the system."})
    ...
    await connection.query("UPDATE usuario SET contrasena=? WHERE email=?", [contrasena, email])
    ...
    return sendEmailToUser(email, asunto, mensaje, true)
  } catch (error) {
    console.log(error)
    return res.status(500).send(error.message)
  }
...
export const confirmAccount = async (req, res) => {
  try {
    ...
    if (token !== undefined)
    {
      ...
      await connection.query("UPDATE usuario SET cuentaActivada='Si' WHERE idUsuario=?", id)
